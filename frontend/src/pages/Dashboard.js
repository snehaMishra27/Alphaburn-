...
//         console.log("First workout object:", res.data.workouts[0]);
...
//   const last7DaysData = data.filter(item => new Date(item.date) >= sevenDaysAgo);

//   // ===== TOP STATS =====
//   const totalWorkouts = last7DaysData.length;
//   const totalCalories = last7DaysData.reduce((acc, item) => acc + Number(item.calories || 0), 0);
...
//     const match = last7DaysData.find(item => {
//       const itemDate = new Date(item.date).toLocaleDateString("en-IN", { day: "2-digit", month: "short" });
//       return itemDate === label;
//     });

//     chartData.push({
//       date: label,
//       calories: match ? Number(match.calories || 0) : 0,
//       sleep: match ? Number(match.sleepDuration || match.sleep || 0) : 0,
//       weight: match ? Number(match.weight || 0) : 0,
//     });
//   }
...
//           <h2 className="mb-0 text-white">Your Fitness Dashboard 💪</h2>
...
//             <div className={`card shadow-sm ${dark ? "bg-secondary text-white" : ""}`}>
//               <div className="card-body">
//                 <h5 className="text-center mb-3">Weekly Trends 💤⚖️</h5>
